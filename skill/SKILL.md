...
- This skill assumes a Cowork-style session with WebFetch (arbitrary URLs) and the Artifact `db` capability. On a plainer Claude surface without those tools, the sourcing and scoring logic still works (search for candidates and verify them via whatever fetch capability is available (search-then-fetch worked well as a substitute for direct API enrichment in testing), favoring corroborated evidence over unverifiable claims) but present results as a table in the conversation instead of a live tracker, and tell the user plainly that this run won't persist or be re-runnable as a tracker.
